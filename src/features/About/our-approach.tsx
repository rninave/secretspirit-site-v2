...
const OurApproach: React.FC<OurApproachProps> = ({
  title = "Our Approach",
  description = "We don't just solve problems; we anticipate them. By proactively identifying potential challenges, we empower our team to develop innovative and effective solutions, ensuring we're always prepared.",
  topCards,
  bottomCards = [],
}) => {
    ...
    <div className={`grid grid-cols-1 md:grid-cols-${cols} gap-6 ${maxWidth ? `max-w-${maxWidth} mx-auto` : ''} mb-6`}>
      {cards.map((card) => (
        <Reveal key={card.title}>
          <div className="bg-[#FFF6F4] rounded-xl p-6 shadow-md border min-h-56 border-divider">
            <h4 className="text-[16px] md:text-lg font-bold text-heading font-heading leading-[100%] mb-3">{card.title}</h4>
            <div className="w-12 h-0.5 shadow-about-card bg-primary mb-4" />
